      ...
      return [
        {
          ...
          content:
            ...
            "译文正文不要使用引用块（>）、代码块等 markdown 装饰，纯文本输出。",
        },
  ...
  },
  {
    ...
    buildMessages(text) {
      return [
        {
          role: "system",
          content: `${SYSTEM_BASE} 当前任务：解释用户选中的内容——它是什么/为什么重要/关键点。用 2-4 段短文，术语首次出现给简明定义。如果内容不足以支撑解释（过于零散、缺乏上下文或语义不明），不要猜测，直接说明内容不足并请用户提供更完整的文本。`,
        },
        { role: "user", content: text },
      ];
    },
      ...
      return [
        {
          role: "system",
          content:
            "你是摘要助手。将选中内容提炼为要点：≤5 条 bullet，每条 ≤20 字，按重要性排序。只输出要点，不加前言总结语。如果内容不足以总结（过于零散或缺乏有效信息），不要强行归纳，直接说明内容不足并请用户提供更完整的文本。",
        },
        { role: "user", content: text },
      ];
    },
