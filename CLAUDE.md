...
**拾趣 (Magpie)**：划词即理解的 AI 阅读伴侣。macOS MVP（Windows 为 M2，接口已预留）。Tauri 2 + Rust + React/TS。文档是权威规格：`docs/01-PRD` 至 `docs/07-执行计划`，其中 `docs/02 §3.2` 是 Rust↔TS 事件/命令契约表——改协议须同步文档。
...
├─ main (index.html)  设置/自检        ├─ capture/  划词捕获（最高风险模块）
│    └─ invoke: get/save_settings      ├─ floating.rs  浮动条窗口定位/尺寸/显隐
├─ floating (floating.html) 浮动条     ├─ settings.rs  JSON 配置 + 黑名单
│    └─ 动作注册表(ACTIONS)→prompt     ├─ ai.rs  OpenAI 协议 SSE 哑管道（无 AI 逻辑）
│    └─ aiChat() → Channel 流          └─ main.rs  setup/命令注册/事件分发
```
...
- 平台分发：`capture/mod.rs` 用 `#[cfg]` 选 macos/other 后端，两后端同一 `start(tx, debounce_ms)` 签名；Windows 实现进 `capture/windows.rs` 时照此接入，勿在 worker 层加平台分支。

## 约定

- 决策记录（为什么不用 Electron/LangChain、为什么无剪贴板等）见 `docs/06-技术选型.md`（ADR），翻案先改 ADR。
- 产品边界：不做程序员向功能（报错/代码解释）；划词全程不读写剪贴板；捕获失败静默。
- API Key 目前明文存 `settings.json`（ADR-07，M2 迁 keyring），设置页已向用户明示——不要在别处引入新的密钥落盘点。
- 文档与 UI 文案用简体中文；代码注释密度向现有文件看齐。
