...
> v0.1 · 2026-09 · 配套 [01-PRD](./01-PRD-MVP.md) / [03-划词捕获](./03-模块设计-划词捕获.md) / [04-窗口交互](./04-模块设计-窗口与交互.md) / [05-AI服务层](./05-模块设计-AI服务层.md) / [06-技术选型](./06-技术选型.md)

## 1. 总体原则

1. **Rust 管不变的，TS 管多变的**：系统层（捕获/窗口/存储/传输）放 Rust——五年不随 AI 演进变化；AI 编排（动作、prompt、流式渲染、未来 agent）放 TS（WebView 内）——蹭 npm 生态的迭代速度。
2. **无痕划词**：全程不读/写剪贴板，数据获取只走无障碍 API。
...
sequenceDiagram
    ...
    Note over W: 200ms 防抖 + 黑名单/自身PID/重复选区过滤
    W->>W: AX 查询选中文本（无剪贴板）
...
| 命令 | 参数 | 返回 |
|---|---|---|
| `show_floating_bar` | `x, y` | 定位（含防出屏钳制）并显示 |
...
| `capture_status` | — | `{granted, platform}` |
...
> 安全注记：MVP 将 Key 明文存于本机配置文件（与竞品普遍水平一致）；M2 迁移到系统钥匙串（keyring crate），已在 [05](./05-模块设计-AI服务层.md) 留有接口位。文档与 UI 需明示此现状。
...
| 阶段 | 增量 | 架构落点 |
|---|---|---|
| M2 | Windows UIA 捕获 | `capture/` 下平台后端 trait 已抽象，见 [03](./03-模块设计-划词捕获.md) |
...
| M3 | 知识库（划词收藏+RAG） | Rust 增 `kb` 模块（sqlite + sqlite-vec + 入库管道）；收藏命令入动作注册表 |
...
| 风险 | 等级 | 对策 |
|---|---|---|
| AX 在部分应用拿不到文本 | 高（固有） | 静默失败 + 自检页预期管理 + 应用黑名单；必要时未来加默认关闭的剪贴板兜底 |
