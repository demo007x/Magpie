...
- **九个动作** — 翻译 / 解释 / 总结（AI 生成、流式输出）+ 复制 / 搜索 / 打开链接 / 写邮件 / 复制验证码 / 复制号码，顺序与开关可在设置页调整
- **上下文智能动作** — 自动识别选中文本中的链接、邮箱、验证码、电话号码：单个实体一键直达（打开 / 写邮件 / 复制），多个实体进面板分组批量操作，普通文本不会误弹
...
- 自定义 AI 动作（用户自写 prompt 模板）与多步骤 workflow
- 划词收藏一键接入 Notion / Obsidian / flomo / 备忘录等用户已有的笔记应用（接入而非自建）
