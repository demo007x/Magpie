...
- **全局划词** — 基于 macOS 无障碍 API（CGEventTap + AX）捕获选中文本；拖选或双击即触发，200ms 防抖，同一选区不重复弹出
- **浮动条** — 跟随鼠标弹出、自动防出屏、不抢焦点的毛玻璃胶囊，支持明暗主题
- **九个动作** — 翻译 / 解释 / 总结（AI 流式输出）+ 复制 / 搜索 / 打开链接 / 写邮件 / 复制验证码 / 复制号码，顺序与开关可在设置页调整
- **上下文智能动作** — 自动识别选中文本中的链接、邮箱、验证码、电话号码：单个实体一键直达（打开 / 写邮件 / 复制），多个实体进面板分组批量操作；识别遵循强一致原则（TLD 白名单校验，不规范的形态宁可不识别）
- **权限独立页** — 辅助功能 / 输入监控 / AX 服务健康检查集中一处，未授权时一键引导开启
...
- **应用黑名单** — 终端、密码管理器等敏感应用内划词不触发；从系统面板选择应用（bundle id 精确匹配，Safari 等系统应用也可靠屏蔽）
- **兼容模式** — 对微信、Office 等不支持取词的应用，自动模拟 ⌘C 读取选中文本并还原剪贴板
