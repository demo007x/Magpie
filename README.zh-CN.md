...
<!-- 本节只写「能力」：用户用这个产品能做什么，每条 = 是什么 + 怎么触发，一句为限。
     不放：性能数字（如 xx ms）、交互细节（快捷键、收起方式）、容错与优化描述、
     实现细节（子进程隔离 / 缓存等）——那些归 CHANGELOG 的 Changed / Fixed，或哪里都不去。
     更新时机：能力有大变化才动本节，发版不需要。 -->

- **全局划词** — 在任意应用中选中文字即可触发，拖选或双击均可
- **浮动条** — 跟随鼠标上方弹出、自动防出屏、不抢焦点，支持亮 / 暗 / 跟随系统主题
- **11 个动作** — 翻译 / 解释 / 总结（AI 生成、流式输出）+ 复制 / 搜索 / 打开链接 / 写邮件 / 复制验证码 / 复制号码 + 加入日历 / 打开地图，顺序与开关可在设置页调整
- **上下文智能动作** — 自动识别选中文本中的链接、邮箱、验证码、电话号码、日期、地点：单个实体一键直达（打开 / 写邮件 / 复制 / 加日历 / 地图），多个实体进面板分组批量操作
- **框选识图** — 自定义全局快捷键或托盘菜单触发框选，Vision OCR 识别文字，一步式识图翻译 / 解释 / 总结；钉图（Snipaste 式）可循环回 OCR
- **结果窗** — AI 结果与识别文本开在独立常驻窗口，可钉住、拖动、记忆位置与尺寸
- **自定义 AI 动作** — 名称 + 提示词 + 图标即成胶囊按钮（上限 10 个，与内置动作同池排序），支持试跑预览
...
- **BYOK** — 自带 API Key，预设 DeepSeek，兼容任意 OpenAI 协议端点；翻译 / 解释 / 总结的提示词可自定义
- **菜单栏常驻** — 默认隐藏 Dock 图标，关闭主窗口仅隐藏，捕获继续工作；GitHub 更新检测提示
- **应用黑名单** — 终端、密码管理器等敏感应用内划词不触发，系统应用也能可靠屏蔽
